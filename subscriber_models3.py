...
def process_entity(df, entity, target, png_base_path, subscriber):
    ...
    if os.path.exists(fname):
        ...
        df1 = df[df['DATETIMEUTC'] < "2017-01-01 00:00:00"]
        df2 = df[df['DATETIMEUTC'] >= "2017-01-01 00:00:00"]
        if not check_valid_target (df1, target) or not check_valid_target (df2, target):
            print ">> ERROR in train or test data!", len(df1), len(df2)
        else:
...
if __name__ == "__main__":        
    ...
    print ">> sorting"
    df_main.sort_values(by='DATETIMEUTC', inplace=True)
    print ">> counting"
    vc = df_main[id_column].value_counts()
    ...
    for entity in vc.index:
        dfx = df_main[df_main[id_column]==entity]
        rows = dfx.shape[0]
        if rows >= min_rows:
            if len(summary) % 10 == 0:
                print summary[-10:]
            dfy = dfx[dfx['DATETIMEUTC'] < "2017-02-01 00:00:00"]
            dfz = dfx[dfx['DATETIMEUTC'] >= "2017-02-01 00:00:00"]
            summary.append (("'%s'" % entity,len(dfy),len(dfz)))
        else:
            break
    print summary[-10:]
    ...
    for entity in vc.index[1:]:
    #print "!!!!!!!!!!! TEST VERSION !!!!!!!!!!!!"
    #for entity in ['40470839-1a9d-49c9-9e94-4b13c537f8ab']:
        print ">> %s %s: '%s'" % (target, id_column, entity)
        df_ent = df_main[df_main[id_column]==entity]
        rows = df_ent.shape[0]
        print ">> Rows:", rows
        if rows < min_rows:
            break
        subscriber = get_subscriber(df_ent) if  id_column != 'SUBSCRIBER_NAME' else None
        score = process_entity (df_ent, entity, target, png_path, subscriber)
        if score != None:
